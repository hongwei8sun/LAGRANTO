...
Num_2D_lower = np.zeros((N_lon, N_lat, N_lev, N_day)) 
Num_2D_upper = np.zeros((N_lon, N_lat, N_lev, N_day)) 
...
#                     P_trop = TROP_p[0, ilat, ilon]
                ...
                if In_strat ==1:
                    ...
                    LAT_0 = data[i_parcel,0,2]
                    LEV_0 = data[i_parcel,0,3] # consider the initial injection height

                    # Only consider initial injection between 15S and 15N
                    if abs(LAT_0)<=15:
                        i_LON = find_i_LON(LON)
                        i_LAT = find_i_LAT(LAT)
                        i_LEV = find_i_LEV(LEV_0)
...
with open('./Num_Concnt_data/Num_Concnt_2000_'+Year+'.txt', 'w') as f:
...
# check whether the lat and lon range are same for TROP.nc and traj file
print(f"LON range of TROP.nc is: {np.min(TROP_lon):.1f} to {np.max(TROP_lon):.1f}")
print(f"LON range of traj file is: {np.min(data[:,:,1]):.1f} to {np.max(data[:,:,1]):.1f}")
print('...')
print(f"LAT range of TROP.nc is: {np.min(TROP_lat):.1f} to {np.max(TROP_lat):.1f}")
print(f"LAT range of traj file is: {np.min(data[:,:,2]):.1f} to {np.max(data[:,:,2]):.1f}")
